...
{
private:
  void IPGenerated(Int_t nvtxmc, const Int_t *mother, std::vector<Double_t> &ipmc, const Double_t *xvmc, const Double_t *yvmc, const Double_t *zvmc);
  void KSLGenerated(Int_t nvtxmc, const Int_t *mother, Double_t Kl[9], const Double_t *xvmc, const Double_t *yvmc, const Double_t *zvmc, Double_t Ks[9], Int_t ntmc, const Int_t *pidmc, const Double_t *pxmc, const Double_t *pymc, const Double_t *pzmc);
  void twoTracksFinder(Int_t ntmc, const Int_t *mother, const Int_t *vtxmc, const Int_t *pidmc, std::vector<Double_t> &Knemc, Double_t Kl[9], std::vector<Double_t> &Kchmc, Double_t Ks[9], std::vector<std::vector<Double_t>> &trkMC, const Double_t *pxmc, const Double_t *pymc, const Double_t *pzmc, Int_t mctruth, std::vector<Double_t> &CurvMC, std::vector<Double_t> &PhivMC, std::vector<Double_t> &CotvMC, Int_t &muonAlertPlus, Int_t &muonAlertMinus);
  void GeneratedClusterFinder(Int_t nclu, Int_t ind_gam[4], const Int_t max_count, std::vector<Double_t> &clus_diff, std::vector<std::vector<Double_t>> &cluster_rec, std::vector<std::vector<Double_t>> &pgammaMC, Int_t mc_ind[4], std::vector<bool> &clus_time, Int_t min_ind[4], Double_t &clus_diff_min, std::vector<Int_t> &good_clus_ind);
  void ClusterVariableFinder(Int_t ntmc, const Int_t *mother, const Int_t *vtxmc, const Int_t *pidmc, std::vector<std::vector<Double_t>> &pgammaMC, Int_t &count, const Double_t *pxmc, const Double_t *pymc, const Double_t *pzmc, Double_t neu_vtx[3], std::vector<Double_t> &Knemc, std::vector<Double_t> &region, KLOE::CylinderIntersection &CylIndObj, std::vector<Double_t> &cluster, std::vector<Double_t> &ipmc);

  struct MCNode
  {
    Int_t particleID; // PID of this particle instance
    Int_t motherID;    // PID (species) of its parent
    Int_t vertexID;    // production vertex of this particle
    Int_t parentKine;  // kine number of the specific parent instance
  };
  std::unordered_map<Int_t /*kine*/, MCNode> nodes;

  bool IsPrimaryFromPhi(Int_t kineId) const;
  bool IsFromKs(Int_t kineId) const;
  bool IsFromKl(Int_t kineId) const;
