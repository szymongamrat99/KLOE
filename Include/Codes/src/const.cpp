...
namespace KLOE
{
  ...
  const std::map<Int_t, TString> otherChannName = {
      ...
      {3, "K+K-pi+pi0mu-"},
      {4, "K+K-mu+pi-pi0"},
      {5, "KLKS3pi0pi+pi-gamma"},
      {6, "KLKSpi+pi-pi0"},
      {7, "KSKLlambda"},
      {8, "K+K-pi+pi0pi0pi-pi0pi0"},
      {9, "K+K-pi+pi0pi-pi0pi0"},
      {10, "K+K-pi+pi0pi0pi-pi0"},
      {11, "K+K-positronpi-pi0"},
      {12, "K+K-mu+pi0pi-pi0pi0"},
      {13, "Omega_pi0_pi+pi-pi0"},
      {14, "K+K-mu+pi-pi0pi0"},
      {15, "K+K-pi+pi0pi0_muminus"},
      {16, "KSKL3pi0_semileptonic"},
      {17, "KSKL3pi0_2pi0"},
      {18, "K+K-pi0pi0piplus_pi0electronpositron"},
      {19, "K+K-pi0piplus_pi0electronpositron"},
      {20, "Other"}};
  ...
  const std::map<TString, TString> otherChannTitle = {
      ...
      {"KLKS3pi0pi+pi-gamma", "K_{L}(K_{S})#rightarrow3#pi^{0}#pi^{+}#pi^{-}#gamma"},
      ...
      {"Omega_pi0_pi+pi-pi0", "#omega#pi^{0}#rightarrow#pi^{+}#pi^{-}#pi^{0}"},
...
namespace Utils
{
  ...
  void InitializeVariables(ErrorHandling::ErrorLogs &logger)
  {
    ...
    for (const auto &entry : summaryMap)
    {
      ...
      setConstant(properties_opt, "S013EPI/2025", PhysicsConstants::Im_nonCPT, CPTStatus::NON_CPT, M_PI / 180.);
      setConstant(properties_opt, "S013F+-/2025", PhysicsConstants::phi_pm_nonCPT, CPTStatus::NON_CPT, M_PI / 180.);
      setConstant(properties_opt, "S013FOO/2025", PhysicsConstants::phi_00_nonCPT, CPTStatus::NON_CPT, M_PI / 180.);
    }
  }
  ...
  void JsonFieldLookupDouble(nlohmann::json &jsonObj, const std::string fieldPath, double &value, ErrorHandling::ErrorLogs &logger)
  {
    ...
    try
    {
      value = jsonObj.at(nlohmann::json::json_pointer(pointerString)).get<double>();
      ErrorHandling::InfoCodes infoCode = ErrorHandling::InfoCodes::VARIABLES_INITIALIZED;
      logger.getLog(infoCode, Form("Successfully accessed JSON field: %s. Value: %f", fieldPath.c_str(), value));
    }
    catch (const nlohmann::json::out_of_range &e)
    {
      ErrorHandling::ErrorCodes errorCode = ErrorHandling::ErrorCodes::INITIALIZATION_FAILED;
  ...
  void JsonFieldLookupLong(nlohmann::json &jsonObj, const std::string fieldPath, long &value, ErrorHandling::ErrorLogs &logger)
  {
    ...
    try
    {
      value = jsonObj.at(nlohmann::json::json_pointer(pointerString)).get<long>();
      ErrorHandling::InfoCodes infoCode = ErrorHandling::InfoCodes::VARIABLES_INITIALIZED;
      logger.getLog(infoCode, Form("Successfully accessed JSON field: %s. Value: %ld", fieldPath.c_str(), value));
    }
    catch (const nlohmann::json::out_of_range &e)
    {
      ErrorHandling::ErrorCodes errorCode = ErrorHandling::ErrorCodes::INITIALIZATION_FAILED;
