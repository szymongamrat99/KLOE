...
namespace KLOE
{
  ...
  template <typename F, typename T>
  ErrorHandling::ErrorCodes ChargedVtxRec<F, T>::findKchRec(F *KchRec, F *trk1, F *trk2, Int_t *vtaken, ErrorHandling::ErrorLogs &logger)
  {
  ...
  template <typename F, typename T>
  ErrorHandling::ErrorCodes ChargedVtxRec<F, T>::findKchRec(std::vector<F> &KchRec, std::vector<F> &trk1, std::vector<F> &trk2, std::vector<Int_t> &vtaken, ErrorHandling::ErrorLogs &logger)
  {
    F mom_vec1Tmp[4], mom_vec2Tmp[4], KchTmp[9];
    std::vector<Int_t> ivTmp(_iv, _iv + MaxNumTrkV);
    std::map<Int_t, Int_t> mapTmp = pm00::CountRepeatingElements(ivTmp);
