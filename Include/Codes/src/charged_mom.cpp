...
{
  template <typename F>
  ChargedVtxRec<F>::ChargedVtxRec(Int_t &nv, Int_t &ntv, UChar_t *iv, F *IP, F *CurV, F *PhiV, F *CotV, F *xv, F *yv, F *zv, Int_t &mode) : _iv(iv), _nv(nv), _ntv(ntv), _IP(IP), _CurV(CurV), _PhiV(PhiV), _CotV(CotV), _xv(xv), _yv(yv), _zv(zv), _mode(mode){};

  template <typename F>
  ChargedVtxRec<F>::ChargedVtxRec() : _iv(nullptr), _nv(def), _ntv(def), _IP(nullptr), _CurV(nullptr), _PhiV(nullptr), _CotV(nullptr), _xv(nullptr), _yv(nullptr), _zv(nullptr), _mode(def){};

  template <typename F>
  void ChargedVtxRec<F>::charged_mom(F Curv, F Phiv, F Cotv, F *mom_vec, Int_t mode)
  {
  ...
  template <typename F>
  void ChargedVtxRec<F>::charged_mom(Int_t &i, F *mom_vec)
  {
  ...
  template <typename F>
  void ChargedVtxRec<F>::findKchRec(F *KchRec, F *trk1, F *trk2, Int_t *vtaken, Int_t &errFlag)
  {
    F
        mom_vec1Tmp[4],
        mom_vec2Tmp[4],
    ...
    {
      // Check if two tracks from a vertex
      if (mapTmp[_iv[i]] == 2)
      {
        for (Int_t j1 = 0; j1 < _ntv - 1; j1++)
          for (Int_t j2 = j1 + 1; j2 < _ntv; j2++)
          {
            ...
            {
              // Check if opposite signs of tracks
  ...
  template <typename F>
  void ChargedVtxRec<F>::findKSLRec(Int_t kaonFlag, Int_t KSvtx, F *KchRec, F *trk1, F *trk2, Int_t *vtaken, Int_t &errFlag)
  {
    F
        mom_vec1Tmp[4],
        mom_vec2Tmp[4],
    ...
    for (Int_t i = 0; i < _nv; i++)
    {
      // Check if two tracks from a vertex and length is good
      ...
      if (kaonFlag == 16)
      {
        distFlag = (cyl_vol[0] < 10. && cyl_vol[2] < 20.);
      }
      else if (kaonFlag == 10)
      {
        distFlag = (KSvtx != i);
      ...
      {
        std::cout << "Bad flag composition!" << std::endl;
        break;
      }
  ...
  template <typename F>
  void ChargedVtxRec<F>::findKClosestRec(F *KchRec, F *trk1, F *trk2, Int_t *vtaken, Int_t &errFlag)
  {
    F
        mom_vec1Tmp[4],
        mom_vec2Tmp[4],
  ...
  template <typename F>
  Int_t ChargedVtxRec<F>::KaonMomFromBoost(F *pKaon, F *pboost, F *pKaonBoost)
  {
    std::string
        name = "";
  ...
  template <typename F>
  Int_t ChargedVtxRec<F>::IPBoostCorr(F *X_line, F *vec_line, F *X_plane, F *vec_plane, F *int_point)
  {
    std::string
        name = "";
  ...
  template class ChargedVtxRec<Float_t>;
  template class ChargedVtxRec<Double_t>;
