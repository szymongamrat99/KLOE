...
Int_t TrilaterationNeurecKinfit(TChain &chain, Controls::DataType &dataType, ErrorHandling::ErrorLogs &logger, KLOE::pm00 &Obj)
{
  ...
  // Cluster vars
  Int_t nclu;
  UChar_t mctruth, mcflag, ncll[50];
      ...
      for (Int_t j1 = 0; j1 < nclu - 3; j1++)
        for (Int_t j2 = j1 + 1; j2 < nclu - 2; j2++)
          for (Int_t j3 = j2 + 1; j3 < nclu - 1; j3++)
            ...
            {
              X.ResizeTo(N_free + N_const);
              X_init.ResizeTo(N_free + N_const);
              V.ResizeTo(N_free + N_const, N_free + N_const);
              V_init.ResizeTo(N_free + N_const, N_free + N_const);
              ...
              for (Int_t k = 0; k < 4; k++)
              {
                R.SetClu(k, cluster[0][ncll[ind_gam[k]] - 1],
                         cluster[1][ncll[ind_gam[k]] - 1],
                         cluster[2][ncll[ind_gam[k]] - 1],
                         cluster[3][ncll[ind_gam[k]] - 1],
                         cluster[4][ncll[ind_gam[k]] - 1]);
              ...
              clusterEnergy = cluster[4][ncll[ind_gam[0]] - 1] > KLOE::MIN_CLU_ENE &&
                              cluster[4][ncll[ind_gam[1]] - 1] > KLOE::MIN_CLU_ENE &&
                              cluster[4][ncll[ind_gam[2]] - 1] > KLOE::MIN_CLU_ENE &&
                              cluster[4][ncll[ind_gam[3]] - 1] > KLOE::MIN_CLU_ENE;

              for (Int_t k = 0; k < 4; k++)
              {
                cond_clus[k] =
                    cluster[3][ncll[ind_gam[k]] - 1] > 0 &&
                    cluster[0][ncll[ind_gam[k]] - 1] != 0 &&
                    cluster[1][ncll[ind_gam[k]] - 1] != 0 &&
                    cluster[2][ncll[ind_gam[k]] - 1] != 0;

                if (k < 2)
                  cond_time_clus[k] = S.sol[k][3] < cluster[3][ncll[ind_gam[0]] - 1] &&
                                      S.sol[k][3] < cluster[3][ncll[ind_gam[1]] - 1] &&
                                      S.sol[k][3] < cluster[3][ncll[ind_gam[2]] - 1] &&
                                      S.sol[k][3] < cluster[3][ncll[ind_gam[3]] - 1];
              }
              ...
              if (cond_tot && (cond_time_clus[0] || cond_time_clus[1]))
              {
                for (Int_t k = 0; k < 4; k++)
                {
                  Param[k * 5] = cluster[0][ncll[ind_gam[k]] - 1];
                  Param[k * 5 + 1] = cluster[1][ncll[ind_gam[k]] - 1];
                  Param[k * 5 + 2] = cluster[2][ncll[ind_gam[k]] - 1];
                  Param[k * 5 + 3] = cluster[3][ncll[ind_gam[k]] - 1];
                  Param[k * 5 + 4] = cluster[4][ncll[ind_gam[k]] - 1];
                ...
                for (Int_t k1 = jmin; k1 <= jmax; k1++)
                {
                  ...
                  cond_time_clus[0] = 1;//S.sol[0][3] < X(3) &&
                                      //S.sol[0][3] < X(8) &&
                                      //S.sol[0][3] < X(13) &&
                                      //S.sol[0][3] < X(18);

                  cond_time_clus[1] = 1;//S.sol[1][3] < X(3) &&
                                      //S.sol[1][3] < X(8) &&
                                      //S.sol[1][3] < X(13) &&
                                      //S.sol[1][3] < X(18);
                  ...
                  if (abs(CHISQRTMP) < abs(CHISQRMIN))
                  {
                    if (cond_time_clus[0] && value[0] < value[1])
                    {
                      ...
                      // std::cout << "CHISQR: " << CHISQRTMP << std::endl; 
