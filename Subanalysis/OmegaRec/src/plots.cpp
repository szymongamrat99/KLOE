...
int plots(TChain &chain, Short_t &loopcount, Short_t &numOfConstraints, Short_t &jmin, Short_t &jmax, Controls::DataType &dataType, ErrorHandling::ErrorLogs &logger)
{
	...
	tree_omega->SetBranchAddress("Kchrecomega", Kchrec);
	...
	for (Int_t i = 0; i < channNum; i++)
		for (Int_t j = 0; j < 7; j++)
		{
			...
			if (j < 2)
			{
				...
				hist_control_chann[i].push_back(new TH1D(hist_name, "", 50.0, 0.0, 350.0));
				hist_name = "hist_pm_IP_" + std::to_string(i) + std::to_string(j);
				hist_pm_IP[i].push_back(new TH1D(hist_name, "", 50.0, 0.0, 200.0));
				hist_name = "hist_00_IP_" + std::to_string(i) + std::to_string(j);
				hist_00_IP[i].push_back(new TH1D(hist_name, "", 50.0, 0.0, 200.0));
			}
			else if (j == 2)
	...
	for (Int_t i = 0; i < nentries; i++)
	{
		...
		if (baseKin.mctruth == 4 && doneOmega == 1)
		{
			hist_control[0]->Fill(Kchrec[6] - ip_avg[0]);
			hist_control[1]->Fill(Kchrec[7] - ip_avg[1]);
			hist_control[2]->Fill(Kchrec[8] - ip_avg[2]);
		}
		...
		// cond[3] = abs(Kchrec[6] - ip_avg[0]) < sigmas_ip * 0.8;
		// cond[4] = abs(Kchrec[7] - ip_avg[1]) < sigmas_ip * 0.8;
		// cond[5] = abs(Kchrec[8] - ip_avg[2]) < sigmas_ip * 1.0;

		cond[0] = 1;//rho_pm_IP < 1;
		cond[1] = 1;//rho_00_IP < 1;
		cond[2] = 1;//abs(Kchrec[8] - ip_avg[2]) < sigmas_ip * 1.0;
		cond[3] = 1;//abs(neu_vtx_avg[2] - Kchrec[8]) < sigmas_ip * 1.0;

		cond_tot = cond[0] && cond[1] && cond[2] && cond[3];

		if (cond_tot && doneOmega == 1)
		{
			if (baseKin.mctruth == 1)
			{
	...
	for (Int_t i = 0; i < 3; i++)
	{
		...
		triple_fit->SetParLimits(0, 0.0, 10000.0 * parameter[0]);
		triple_fit->SetParLimits(3, 0.0, 10000.0 * parameter[0]);
		triple_fit->SetParLimits(6, 0.0, 10000.0 * parameter[0]);
		...
		result = hist_control[i]->Fit(triple_fit,"SF", "", -5.0, 5.0);
		...
		hist_control[i]->GetYaxis()->SetRangeUser(0.0, hist_control[i]->GetMaximum() + 3000);
		hist_control[i]->Draw();
		fit_text->Draw();
