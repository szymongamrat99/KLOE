						...
						if (cutter.PassCut(2) && cutter.PassCut(3))
						{
							...
							else
							{
								...
								else
								{
									...
									// Pairing of photons to pions
									Obj.PhotonPairingToPi0(gamma_mom_final, 4, bestPairingIndex);

									for (Int_t i = 0; i < 4; i++)
									{
										baseKin.pi01[i] = gamma_mom_final[bestPairingIndex[0]][i] + gamma_mom_final[bestPairingIndex[1]][i];
										baseKin.pi02[i] = gamma_mom_final[bestPairingIndex[2]][i] + gamma_mom_final[bestPairingIndex[3]][i];
									}

									baseKin.pi01[4] = pow(baseKin.pi01[0], 2) + pow(baseKin.pi01[1], 2) + pow(baseKin.pi01[2], 2);
									baseKin.pi01[5] = sqrt(pow(baseKin.pi01[3], 2) - baseKin.pi01[4]);
									baseKin.pi01[4] = sqrt(baseKin.pi01[4]);

									baseKin.pi02[4] = pow(baseKin.pi02[0], 2) + pow(baseKin.pi02[1], 2) + pow(baseKin.pi02[2], 2);
									baseKin.pi02[5] = sqrt(pow(baseKin.pi02[3], 2) - baseKin.pi02[4]);
									baseKin.pi02[4] = sqrt(baseKin.pi02[4]);
									///////////////////////////////////////////////////////////////////
