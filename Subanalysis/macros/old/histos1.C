...
void histos1::Begin(TTree * /*tree*/)
{
  ...
  //    for(Int_t j = 0; j < KLOE::channNum-2; j++) canvaproj[i][j] = new TCanvas((channName[j] + " canvaproj" + to_string(i)).c_str(), "", 750, 750);
   ...
   for(Int_t j = 0; j < KLOE::channNum; j++) hist_pipi[6][j] = new TH1F(channName[j] + "6pipi", "", 12, PhysicsConstants::mK0-20.0, PhysicsConstants::mK0+20.0);
   for(Int_t j = 0; j < KLOE::channNum; j++) hist_pipi[7][j] = new TH1F(channName[j] + "7pipi", "", 12, PhysicsConstants::mK0-20.0, PhysicsConstants::mK0+20.0);
...
void histos1::Terminate()
{
   ...
   for(Int_t i = 0; i < 8; i++)
   {
	fit_semi[i] = new TFractionFitter(hist_semi[i][KLOE::channNum-2],mc_semi[i]);
   ...
   TLine *line_down = new TLine(PhysicsConstants::mK0-cut_value,0,PhysicsConstants::mK0-cut_value,1E8);
   TLine *line_up = new TLine(PhysicsConstants::mK0+cut_value,0,PhysicsConstants::mK0+cut_value,1E8);
   line_up->SetLineWidth(3);
   line_down->SetLineWidth(3);
   line_up->SetLineStyle(10);
   line_down->SetLineStyle(10);

   TBox *box_down = new TBox(PhysicsConstants::mK0-20.0,0,PhysicsConstants::mK0-cut_value,1E8);
   TBox *box_up = new TBox(PhysicsConstants::mK0+cut_value,0,PhysicsConstants::mK0+20.0,1E8);
   ...
   hist_semi[0][KLOE::channNum-1]->GetYaxis()->SetRangeUser(0.1,1E4);
