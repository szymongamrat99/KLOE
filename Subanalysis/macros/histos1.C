#define histos1_cxx
// The class definition in histos1.h has been generated automatically
...
//
// root> T->Process("histos1.C")
// root> T->Process("histos1.C","some options")
// root> T->Process("histos1.C+")
//
...
void histos1::Begin(TTree * /*tree*/)
{
  ...
  //    for(Int_t j = 0; j < channNum-2; j++) canvaproj[i][j] = new TCanvas((channName[j] + " canvaproj" + to_string(i)).c_str(), "", 750, 750);
   ...
   for(Int_t j = 0; j < channNum; j++) hist_pipi[6][j] = new TH1F(channName[j] + "6pipi", "", 12, mK0-20.0, mK0+20.0);
   for(Int_t j = 0; j < channNum; j++) hist_pipi[7][j] = new TH1F(channName[j] + "7pipi", "", 12, mK0-20.0, mK0+20.0);
...
void histos1::SlaveBegin(TTree * /*tree*/)
{
...
Bool_t histos1::Process(Long64_t entry)
{
   ...
   Float_t tpm, t00, DeltaT, tks, tkl, k_betaks, k_betakl, k_pathks, k_pathkl;
   ...
   cuts_signal_neutral_cs[0] = 1;//(abs(fourKnetri[5] - mK0) < 76);
   cuts_signal_neutral_cs[1] = 1;//(trcv_sum > -5);

   cuts_signal_charged_cs[0] = 1;//(abs(Kchrec[5] - mK0) < 1.2);
   ...
   if(sqrt(pow(Kchrec1[6] - *Bx,2) + pow(Kchrec1[7] - *By,2) + pow(Kchrec1[8] - *Bz,2)) < sqrt(pow(Kchrec2[6] - *Bx,2) + pow(Kchrec2[7] - *By,2) + pow(Kchrec2[8] - *Bz,2)))
   {
	...
	tks = k_pathks/(cVel*k_betaks*tau_S_nonCPT);
        tkl = k_pathkl/(cVel*k_betakl*tau_S_nonCPT);	
   }
   else
   {
   ...
   {
	   if(*mctruth == 1 || *mctruth == 3 || *mctruth == 4 || *mctruth == 5 || *mctruth == 6 || *mctruth_pipi == 1 || *mctruth == 7) hist_semi_nocuts->Fill(DeltaT);
   ...
   if(*mctruth == 7) entries[6]++;
      ...
      if(tot_cuts_neutral)
      {
         ...
         if(*mctruth == 1)
         {
            hist_semi[0][0]->Fill(DeltaT,tot_br_semi);
	    ...
	    entries_sel[0][0]++;
         ...
         if(*mctruth == 3)
         {
            hist_semi[0][1]->Fill(DeltaT,tot_br_semi);
	    ...
	    entries_sel[0][1]++;
         ...
         if(*mctruth == 4)
         {
            hist_semi[0][2]->Fill(DeltaT,tot_br_semi);
	    ...
	    entries_sel[0][2]++;
         ...
         if(*mctruth == 5)
         {
            hist_semi[0][3]->Fill(DeltaT,tot_br_semi);
	    ...
	    entries_sel[0][3]++;
         ...
         if(*mctruth == 6)
         {
            hist_semi[0][4]->Fill(DeltaT,tot_br_semi);
	    ...
	    entries_sel[0][4]++;
         ...
         if(*mctruth_pipi == 1)
         {
            hist_semi[0][5]->Fill(DeltaT,tot_br_semi);
	    ...
	    entries_sel[0][5]++;
         ...
         if(*mctruth == 7)
         {
            hist_semi[0][6]->Fill(DeltaT,tot_br_semi);
	 ...
	 if(*mcflag == 0)
         {
            hist_semi[0][7]->Fill(DeltaT,tot_br_semi);
   ...
   //Pipi selection
   if(donepipi[0] == 1 && donepipi[1] == 1 && abs(Kchrec1[5] - mK0) < 2 /*&& *done4 == 1*/)
   {
      ...
      if(tot_cuts_charged)
      {
         ...
         if(*mctruth == 1)
         {
            hist_pipi[0][0]->Fill(tks-tkl);
            hist_pipi[1][0]->Fill(k_pathks);
            hist_pipi[2][0]->Fill(k_pathkl);
         ...
         if(*mctruth == 3)
         {
            hist_pipi[0][1]->Fill(tks-tkl);
            hist_pipi[1][1]->Fill(k_pathks);
            hist_pipi[2][1]->Fill(k_pathkl);
         ...
         if(*mctruth == 4)
         {
            hist_pipi[0][2]->Fill(tks-tkl);
            hist_pipi[1][2]->Fill(k_pathks);
            hist_pipi[2][2]->Fill(k_pathkl);
         ...
         if(*mctruth == 5)
         {
            hist_pipi[0][3]->Fill(tks-tkl);
            hist_pipi[1][3]->Fill(k_pathks);
            hist_pipi[2][3]->Fill(k_pathkl);
         ...
         if(*mctruth == 6)
         {
            hist_pipi[0][4]->Fill(tks-tkl);
            hist_pipi[1][4]->Fill(k_pathks);
            hist_pipi[2][4]->Fill(k_pathkl);
         ...
         if(*mctruth_pipi == 1)
         {
            hist_pipi[0][5]->Fill(tks-tkl);
            hist_pipi[1][5]->Fill(k_pathks);
            hist_pipi[2][5]->Fill(k_pathkl);
         ...
         if(*mctruth == 7)
         {
            hist_pipi[0][6]->Fill(tks-tkl);
            hist_pipi[1][6]->Fill(k_pathks);
            hist_pipi[2][6]->Fill(k_pathkl);
...
void histos1::Terminate()
{
   ...
   for(Int_t i = 0; i < 8; i++)
   {
	fit_semi[i] = new TFractionFitter(hist_semi[i][channNum-2],mc_semi[i]);
   ...
   for(Int_t i = 0; i < 8; i++)
   {
      ...
      y1d_semi[i][0] = 0.1;
      y1d_semi[i][1] = ceil(max_counts_semi[i][index_sort_semi[i][0]]) + 2E8;
   }
   ...
   for(Int_t i = 0; i < 8; i++)
   {
      ...
      y1d_three[i][0] = 0.1;
      y1d_three[i][1] = ceil(max_counts_three[i][index_sort_three[i][0]]) + 200.;
   }
   ...
   for(Int_t i = 0; i < 8; i++)
   {
      ...
      y1d_pipi[i][0] = 0.1;
      y1d_pipi[i][1] = ceil(max_counts_pipi[i][index_sort_pipi[i][0]]) + 200.;
   }

   Float_t cut_value = 2; 

   cout << hist_semi[7][4]->GetBinCenter(hist_semi[7][4]->GetMaximumBin()) << endl;

   TLine *line_down = new TLine(mK0-cut_value,0,mK0-cut_value,1E8);
   TLine *line_up = new TLine(mK0+cut_value,0,mK0+cut_value,1E8);
   line_up->SetLineWidth(3);
   line_down->SetLineWidth(3);
   line_up->SetLineStyle(10);
   line_down->SetLineStyle(10);

   TBox *box_down = new TBox(mK0-20.0,0,mK0-cut_value,1E8);
   TBox *box_up = new TBox(mK0+cut_value,0,mK0+20.0,1E8);
   ...
   hist_semi[0][channNum-1]->GetYaxis()->SetRangeUser(0.1,1E4);
   ...
   cout << "Purity semi: " << 100*entries_sel[0][4]/(Float_t)(entries_sel[0][4] + entries_sel[0][0] + entries_sel[0][1] + entries_sel[0][2] + entries_sel[0][3] + entries_sel[0][5] + entries_sel[0][6])<< "%" << endl; 
