...
int comp_of_methods()
{
  ...
  for(Int_t i = 0; i < 24; i++)
  {
    ...
    if(i == 0)
      pulls[i] = new TH1F(id_hist, ";#vec{p}^{tri}_{neu,1} - #vec{p}^{gen}_{neu,1} [MeV/c];Counts", 50, -100, 100);
    else if(i == 1)
      pulls[i] = new TH1F(id_hist, ";#vec{p}^{tri}_{neu,2} - #vec{p}^{gen}_{neu,2} [MeV/c];Counts", 50, -100, 100);
    else if(i == 2)
      pulls[i] = new TH1F(id_hist, ";#vec{p}^{tri}_{neu,3} - #vec{p}^{gen}_{neu,3} [MeV/c];Counts", 50, -100, 100);
    else if(i == 3)
      pulls[i] = new TH1F(id_hist, ";E^{tri}_{neu} - E^{gen}_{neu,3} [MeV];Counts", 50, -10, 10);
    else if(i == 4)
      pulls[i] = new TH1F(id_hist, ";#vec{X}^{tri}_{neu,1} - #vec{X}^{gen}_{neu,1} [cm];Counts", 50, -10, 10);
    else if(i == 5)
      pulls[i] = new TH1F(id_hist, ";#vec{X}^{tri}_{neu,2} - #vec{X}^{gen}_{neu,2} [cm];Counts", 50, -10, 10);
    else if(i == 6)
      pulls[i] = new TH1F(id_hist, ";#vec{X}^{tri}_{neu,3} - #vec{X}^{gen}_{neu,3} [cm];Counts", 50, -10, 10);
    else if(i == 7)
      pulls[i] = new TH1F(id_hist, ";t^{tri}_{neu} - t^{gen}_{neu} [#tau_{S}];Counts", 50, -6, 6);
  }
  ...
  for (Int_t i = 0; i < 4; i++)
  {
    if (i < 2)
    {
      ...
      res_std_hist[i] = new TH1F(name[i] + " std res", "", number_of_points, 0, 40);
      res_tri_hist[i] = new TH1F(name[i] + " tri res", "", number_of_points, 0, 40);
      res_tri_kin_fit_hist[i] = new TH1F(name[i] + " kinfit res", "", number_of_points, 0, 40);
      sigmas_std[i] = new TH2F(name[i] + " sigmas std", "", number_of_points, 0, 40, 25, -10, 10);
      sigmas_tri[i] = new TH2F(name[i] + " sigmas tri", "", number_of_points, 0, 40, 25, -10, 10);
      sigmas_tri_kin_fit[i] = new TH2F(name[i] + " sigmas kinfit", "", number_of_points, 0, 40, 25, -10, 10);
    }
    else if (i == 2)
    {
  ...
  for (Int_t i = 0; i < nentries; i++)
  {
    ...
    if ((mctruth == 1 || mctruth == 2) && done_kinfit == 1)// && TMath::Prob(chi2min, M) > 0.1)
    {
      counts++;
  ...
  TCanvas *cov_mat_canvas = new TCanvas("cov_mat_canvas", "", 790, 790);

  corr_matrix_plot->Draw("COLZ");

  cov_mat_canvas->Print("cov_matrix.png");
