...
int comp_of_methods(Int_t first, Int_t last, Int_t loopcount, const Int_t M, const Int_t range, const Int_t ind_data_mc = 0, Int_t file_num = 0, Double_t cut_prob = 0.0, Double_t time_cut = 100.0)
{
  ...
  TFile *file_corr = new TFile("resolutions_histos.root", "update");
  ...
  TCanvas *canvas[2][100], *canvas_pulls[2][24], *canvas_std[2][100];
  ...
  for (Int_t j = 0; j < 2; j++)
  {
    ...
    for (Int_t i = 0; i < 5; i++)
    {
      ...
      if (i < 2)
      {
        neu_std_hist[j][i] = new TH1F(id_hist + range + file_num + " std coor", "", 100, -200, 200);
        neu_tri_hist[j][i] = new TH1F(id_hist + range + file_num + " tri coor", "", 100, -200, 200);
        neu_tri_kin_fit_hist[j][i] = new TH1F(id_hist + range + file_num + " kinfit coor", "", 100, -165, 165);
        res_std_hist[j][i] = new TH1F(id_hist + range + file_num + " std res", "", number_of_points, 0, 50);
        res_tri_hist[j][i] = new TH1F(id_hist + range + file_num + " tri res", "", number_of_points, 0, 50);
        res_tri_kin_fit_hist[j][i] = new TH1F(id_hist + range + file_num + " kinfit res", "", number_of_points, 0, 50);
        sigmas_std[j][i] = new TH2F(id_hist + range + file_num + " sigmas std", "", number_of_points, 0, 50, 50, -80, 80);
        sigmas_tri[j][i] = new TH2F(id_hist + range + file_num + " sigmas tri", "", number_of_points, 0, 50, 50, -80, 80);
        sigmas_tri_kin_fit[j][i] = new TH2F(id_hist + range + file_num + " sigmas kinfit", "", number_of_points, 0, 50, 50, -80, 80);
      }
      else if (i == 2)
      {
        neu_std_hist[j][i] = new TH1F(id_hist + range + file_num + " std", "", 100, 0, 5);
        neu_tri_hist[j][i] = new TH1F(id_hist + range + file_num + " tri", "", 100, 0, 5);
        neu_tri_kin_fit_hist[j][i] = new TH1F(id_hist + range + file_num + " kinfit", "", 100, 0, 5);
        res_std_hist[j][i] = new TH1F(id_hist + range + file_num + " std res", "", number_of_points, 0, 50);
        res_tri_hist[j][i] = new TH1F(id_hist + range + file_num + " tri res", "", number_of_points, 0, 50);
        res_tri_kin_fit_hist[j][i] = new TH1F(id_hist + range + file_num + " kinfit res", "", number_of_points, 0, 50);
        sigmas_std[j][i] = new TH2F(id_hist + range + file_num + " sigmas std", "", number_of_points, 0, 50, 50, -80, 80);
      ...
      else if (i == 3)
      {
        ...
        res_tri_hist[j][i] = new TH1F(id_hist + range + file_num + " tri res", "", number_of_points, 0, 50);
        res_tri_kin_fit_hist[j][i] = new TH1F(id_hist + range + file_num + " kinfit res", "", number_of_points, 0, 50);
        sigmas_std[j][i] = new TH2F(id_hist + range + file_num + " sigmas std", "", number_of_points, 0, 50, 50, -20, 20);
        sigmas_tri[j][i] = new TH2F(id_hist + range + file_num + " sigmas tri", "", number_of_points, 0, 50, 50, -20, 20);
        sigmas_tri_kin_fit[j][i] = new TH2F(id_hist + range + file_num + " sigmas kinfit", "", number_of_points, 0, 50, 50, -20, 20);
      }
      else if (i == 4)
      {
        neu_std_hist[j][i] = new TH1F(id_hist + range + file_num + " std", "", 100, 0, 5);
        ...
        res_tri_hist[j][i] = new TH1F(id_hist + range + file_num + " tri res", "", number_of_points, 0, 50);
        res_tri_kin_fit_hist[j][i] = new TH1F(id_hist + range + file_num + " kinfit res", "", number_of_points, 0, 50);
        sigmas_std[j][i] = new TH2F(id_hist + range + file_num + " sigmas std", "", number_of_points, 0, 50, 50, -10, 10);
        sigmas_tri[j][i] = new TH2F(id_hist + range + file_num + " sigmas tri", "", number_of_points, 0, 50, 50, -10, 10);
        sigmas_tri_kin_fit[j][i] = new TH2F(id_hist + range + file_num + " sigmas kinfit", "", number_of_points, 0, 50, 50, -10, 10);
      }
    }
  }
  ...
  for (Int_t i = 0; i < nentries; i++)
  {
    ...
    if (mcflag == 1 && (mctruth == 1 || mctruth == 2))
    {
      ...
      v_Kneutri = /*lengthneu_tri / Knetri_kinfit[9];*/ c_vel * sqrt(pow(Knetri_kinfit[0], 2) + pow(Knetri_kinfit[1], 2) + pow(Knetri_kinfit[2], 2)) / Knetri_kinfit[3];
        ...
        if (done_kinfit == 1)
        {
          ...
          if (j == 0)
          {
            ...
            if (isEqual == 0)
            {
              ...
              if (abs(Knetri_kinfit[9] - 0 - t_neumc) < 2.715 / 2.)
                counts_goodzero++;
              else if (Knetri_kinfit[9] - 0 - t_neumc < 3. * 2.715 / 2. && Knetri_kinfit[9] - 0 - t_neumc > 2.715 / 2.)
                counts_goodpone++;
              else if (Knetri_kinfit[9] - 0 - t_neumc < -2.715 / 2. && Knetri_kinfit[9] - 0 - t_neumc > -3. * 2.715 / 2.)
                counts_goodmone++;
              else if (Knetri_kinfit[9] - 0 - t_neumc > 3. * 2.715 / 2. && Knetri_kinfit[9] - 0 - t_neumc < 200.)
                counts_blob++;
            }
            else
            {
          ...
          if (abs(Knetri_kinfit[9] - t_neumc) < 2.715 / 2.)
            bunch_corr = 0;
          else if (Knetri_kinfit[9] - t_neumc < -2.715 / 2. && Knetri_kinfit[9] - t_neumc > -3. * 2.715 / 2.)
            bunch_corr = T0;
          else if (Knetri_kinfit[9] - t_neumc < -3. * 2.715 / 2. && Knetri_kinfit[9] - t_neumc > -5. * 2.715 / 2.)
            bunch_corr = 2 * T0;
          else if (Knetri_kinfit[9] - t_neumc < -5. * 2.715 / 2. && Knetri_kinfit[9] - t_neumc > -7. * 2.715 / 2.)
            bunch_corr = 3 * T0;
          else if (Knetri_kinfit[9] - t_neumc < -7. * 2.715 / 2. && Knetri_kinfit[9] - t_neumc > -9. * 2.715 / 2.)
            bunch_corr = 4 * T0;
  ...
  for (Int_t j = 0; j < 2; j++)
  {
    ...
    for (Int_t k = 0; k < 5; k++)
    {
      for (Int_t i = 1; i <= number_of_points; i++)
      {
        ...
        triple_fit->SetParameters(parameter[0]/2., parameter[1], parameter[2], parameter[0]/2., parameter[1]+6.0, parameter[2]+5.0, parameter[0]/2., parameter[1]-6.0, parameter[2]+5.0);

        triple_fit->SetParLimits(0, 0.01 * sigmas_std[j][k]->ProjectionY("_py", i, i)->Integral(), 10.0 * sigmas_std[j][k]->ProjectionY("_py", i, i)->Integral());
        triple_fit->SetParLimits(3, 0.01 * sigmas_std[j][k]->ProjectionY("_py", i, i)->Integral(), 10.0 * sigmas_std[j][k]->ProjectionY("_py", i, i)->Integral());
        triple_fit->SetParLimits(6, 0.01 * sigmas_std[j][k]->ProjectionY("_py", i, i)->Integral(), 10.0 * sigmas_std[j][k]->ProjectionY("_py", i, i)->Integral());

        triple_fit->SetParLimits(2, 0.001 * sigmas_std[j][k]->ProjectionY("_py", i, i)->GetStdDev(), 4.0 * sigmas_std[j][k]->ProjectionY("_py", i, i)->GetStdDev());
        triple_fit->SetParLimits(5, 0.001 * sigmas_std[j][k]->ProjectionY("_py", i, i)->GetStdDev(), 4.0 * sigmas_std[j][k]->ProjectionY("_py", i, i)->GetStdDev());
        triple_fit->SetParLimits(8, 0.001 * sigmas_std[j][k]->ProjectionY("_py", i, i)->GetStdDev(), 4.0 * sigmas_std[j][k]->ProjectionY("_py", i, i)->GetStdDev());
    ...
    res_std_hist[j][0]->GetYaxis()->SetRangeUser(0.0, 70.0);
    ...
    res_std_hist[j][3]->GetYaxis()->SetRangeUser(0.0, 20.0);
    ...
    res_std_hist[j][4]->GetYaxis()->SetRangeUser(0.0, 20.0);
