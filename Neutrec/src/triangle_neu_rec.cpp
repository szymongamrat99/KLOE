...
void triangle_neurec(int first_file, int last_file, int good_clus) //	arguments are: 1. Number of points
...

{
	...
	TString name = "neuvtx_triangle_rec_" + std::to_string(first_file) + "_" + std::to_string(last_file) + ".root";

	TFile *file = new TFile(name, "recreate");
	TTree *tree = new TTree("h_triangle", "Neu vtx rec with triangle");
	...
	for (Int_t i = 0; i < nentries; i++)
	{
		...
		if (nclu >= 4 && (mctruth == 1))
		{
			...
			std::cout << Knemc[6] << " " << Knemc[7] << " " << Knemc[8] << std::endl << std::endl;
		}
	}
