						...
						for (Int_t j4 = j3 + 1; j4 < nclu; j4++)
						{
							...
							if (clusterEnergy && cond_clus[0] && cond_clus[1] && cond_clus[2] && cond_clus[3] && (cond_time_clus[0] || cond_time_clus[1]))
							{
								...
								for (Int_t k1 = jmin; k1 <= jmax; k1++)
								{
									...
									if(jmin == 0 && jmax == 0) X_init(27) = bunch_corr / Trf;
									...
									cond_time_clus[0] = 1; // S.sol[0][3] < X(3) + T0 && S.sol[0][3] < X(8) + T0 && S.sol[0][3] < X(13) + T0 && S.sol[0][3] < X(18) + T0;

									cond_time_clus[1] = 1; // S.sol[1][3] < X(3) + T0 && S.sol[1][3] < X(8) + T0 && S.sol[1][3] < X(13) + T0 && S.sol[1][3] < X(18) + T0;
