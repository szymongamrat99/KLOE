...
Int_t main(int argc, char *argv[])
{
	...
	Float_t length_ch, time_ch, velocity_ch, gamma_mom_final[4][8], fourKnetri_kinfit[10], iptri_kinfit[3], y_axis[3], ip_tri[2][3], bhabha_mom_fit[4], gamma_mom_tmp[2][4][4], fourKnetri_tmp[2][4], value[range], kaon_vel_tmp[2], dist_tmp[2], ip_tmp[2][3];
		...
		if (nclu >= 4 && (mctruth == 1 || mctruth == 2))
		{
			...
			if (isConverged)
			{
				...
				if (value[0] < value[1])
				{
					...
					for (Int_t l = 0; l < 4; l++)
					{
						gamma_mom_final[l][0] = gamma_mom_tmp[0][l][0];
						gamma_mom_final[l][1] = gamma_mom_tmp[0][l][1];
						gamma_mom_final[l][2] = gamma_mom_tmp[0][l][2];
						gamma_mom_final[l][3] = gamma_mom_tmp[0][l][3];
				...
				else
				{
					...
					for (Int_t l = 0; l < 4; l++)
					{
						gamma_mom_final[l][0] = gamma_mom_tmp[1][l][0];
						gamma_mom_final[l][1] = gamma_mom_tmp[1][l][1];
						gamma_mom_final[l][2] = gamma_mom_tmp[1][l][2];
						gamma_mom_final[l][3] = gamma_mom_tmp[1][l][3];
						gamma_mom_final[l][4] = X_min.GetMatrixArray()[l * 5];
						gamma_mom_final[l][5] = X_min.GetMatrixArray()[l * 5 + 1];
						gamma_mom_final[l][6] = X_min.GetMatrixArray()[l * 5 + 2];
						gamma_mom_final[l][7] = X_min.GetMatrixArray()[l * 5 + 3] - T0;
